...
def extractZipUrl(url: str, index: int = 0) -> str:
    ...
    try:
        ...
        if xml_file.status_code >= 400:
            logger.error(f"Can't download URL, status code = {xml_file.status_code}")
        ...
        logger.exception(f"Failed to find valid ZIP link for index {index}")
        raise e
...
def extractToCSV(elements: list[Element], csv_path: str) -> None:
    ...
    logger.info(f"Successfully created CSV in directory {csv_path}")
